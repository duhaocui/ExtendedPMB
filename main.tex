...
% For instance, if $f_{\mathbb{H}}(\setX)$ is a Gaussian mixture and $\hat{f}(\setX)$ is a Gaussian, then we can minimize the KL divergence (\ref{eq:klmr}) by moment matching.
...
\begin{equation}
    ...
    % \iota\in\hat{\mathbb{I}}\cap\mathbb{I},
...
% \begin{subequations}
% \begin{align}
%     \hat{r}^{\iota} &= \sum_{j\in\mathbb{J}}\mathcal{W}^jr^{j,\mathfrak{L}(\iota)},\\
%     g^{\iota}(\mathbf{x}) &\propto \sum_{j\in\mathbb{J}}\mathcal{W}^jf^{j,\mathfrak{L}(\iota)}(\mathbf{x}).
% \end{align}
% \label{eq:tomerge}
% \end{subequations}
...
% \footnote{The same hold for (\ref{eq:tonew}), (\ref{eq:mstepjason}), (\ref{eq:mstepop}) and (\ref{eq:approxnewtrack})}

% (\ref{eq:approximatedBernoulli}); the proportionality denotes that normalization is required to ensure that the weights of different $f^{j,i}(\mathbf{x})$, i.e., $\mathcal{W}^jr^{j,i}$, sum to one (e.g., see \cite{ArdeshiriGOO:2015}).
...
%   \iota\in\hat{\mathbb{I}}\setminus\mathbb{I},
    % g^{\iota}(\mathbf{X}^{\iota}) \propto \sum_{j\in\mathbb{J}}\mathcal{W}^jf^{\mathfrak{F}(\iota)}(\mathbf{X}^{\mathfrak{F}(\iota)})\Delta_{\mathfrak{F}(\iota)\in\mathcal{P}^j}, \iota\in\hat{\mathbb{I}}\setminus\mathbb{I},
    \label{eq:tonew}
\end{equation}
...
\underset{g}{\arg\min}~D_{\text{KL}} \bigg(  \sum_{j\in\mathbb{J}}\mathcal{W}^j\\\times\sum_{i\in\mathbb{I}^j\setminus\mathbb{I}}f^{j,i}(\mathbf{X}^i)\Delta_{\{(j,i)\}\in\mathbb{L}^{\iota-|\mathbb{I}|}} ||g^{\iota}(\mathbf{X}^{\iota}) \bigg),
    % g^{\iota}(\mathbf{X}^\iota) \propto \sum_{j\in\mathbb{J}}\mathcal{W}^j\sum_{i\in\mathbb{I}^j\setminus\mathbb{I}}f^{j,i}(\mathbf{X}^i)\Delta_{\{(j,i)\}\in\mathbb{L}^{\iota-|\mathbb{I}|}}, \iota\in\hat{\mathbb{I}}\setminus\mathbb{I},
...
% Corresponding mathematical expressions are not presented here due to page limits. 
